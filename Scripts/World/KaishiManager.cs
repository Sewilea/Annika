...
public class KaishiManager : MonoBehaviour
{
    ...
    public Kaishi[] CaveKai, AnimalKai;
    ...
    public void MobCreate()
    {
        for (float x = MobKai.Xstart; x < MobKai.Xfinish; x++)
        {
            for (float y = MobKai.Ystart; y < MobKai.Yfinish; y++)
            {
                ...
                if(!Obje.GetComponent<Kaishiland>().isfull && Mobs.transform.childCount < 12)
                {
                    if (a == 0)
                    {
    ...
    {
        for (int i = 0; i < CoinSc.Content_Animals.transform.childCount; i++)
        {
            GameObject Obje = CoinSc.Content_Animals.transform.GetChild(i).gameObject;
            Enemy Animal = Obje.GetComponent<Enemy>();
            Animal.Product = 1;
        }
    }
}
