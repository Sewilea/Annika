...
public class Enemy : MonoBehaviour
{
    ...
    public bool Walk, EnemyB;
    public float Heart = 5, Speed = 12;
    public float Product;
    public Item Product_item;
    void Start()
    {
        players = GameObject.Find("Annika").gameObject.GetComponent<Player>();
        Player = GameObject.Find("Annika").gameObject;
    ...
    {
        if (Distance < 3 && !EnemyB && Product == 1)
        {
            Product = 0;
            inventory.AddIteminSlots(Product_item, 1);
        }
    }

    private void OnTriggerStay2D(Collider2D collision)
    {
        if(collision.tag == "hitcollider" && players.Hit && EnemyB)
        {
            skills.Fighting_xp++;
            Heart--;
            players.Hit = false;
